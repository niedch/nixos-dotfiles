...
Item {
  ...
  BorderSurface {
    ...
    // Maintain the border correctly (Fixes issue #8778)
    borderSpec: Border.controlSpec(
      root.checked && Border.controlHasWidth("selected") ? "selected" : "normal",
  ...
  MouseArea {
    ...
    // Defer signal emission using Qt.callLater to prevent QML MouseArea destruction 
    // before processing the release event which would lock up the compositor's mouse grab.
    onClicked: {
      if (!root.busy) {
        Qt.callLater(root.toggled)
      }
    }
  }
}
