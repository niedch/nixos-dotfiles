...
Panel {
  ...
  // The actual Bluetooth sequencing is handled by Quickshell's native
  // Bluetooth device API; this map only keeps the panel responsive while
  // BlueZ catches up.
  ...
  function connectDevice(device) {
    ...
    if (device.paired || device.bonded || device.trusted) device.connected = true
    else if (device.pair) device.pair()
  }
  ...
  // disconnects, then removes the BlueZ pairing record; the sequencing is
  // handled by Quickshell's native Bluetooth device API.
