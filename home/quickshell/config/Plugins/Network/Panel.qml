...
Panel {
  ...
  function startSpeedTestPhase(phase) {
    ...
    speedTestStderr = ""
    speedTestProc.command = ["qs-network-speedtest", phase]
  ...
  function setDns(provider) {
    ...
    if (provider === "Custom") {
      Quickshell.execDetached(["ghostty", "--class=org.tui.dns", "-e", "bash", "-lc", root.dnsCommand(provider)])
      root.close()
      return
    }
  ...
  Process {
    id: detailsProc
    command: ["qs-network-status", "--verbose"]
